...
void gripper(int pulse) {
  static unsigned long timer;  // Static variable to keep track of the time intervals
  static int lastPulse;        // Static variable to store the last pulse value

  // Check if enough time has passed to send a new pulse
  if (millis() > timer) {
    if (pulse > 0) {  // If a valid pulse value is passed (greater than zero)
      lastPulse = pulse;  // Store the pulse value to use later if pulse is zero
    } else {
      pulse = lastPulse;  // If no pulse value is passed, use the last valid pulse
    }

    // Activate the gripper with the specified pulse width
    digitalWrite(GRPIN, HIGH);  // Turn the gripper on
    delayMicroseconds(pulse);   // Wait for the pulse duration (in microseconds)
    digitalWrite(GRPIN, LOW);   // Turn the gripper off

    // Update the timer to control the time interval between pulses
    timer = millis() + 20;  // Set the timer for the next pulse (20ms delay)
  }
}
